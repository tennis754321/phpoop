...
class User extends Config {
    ...
    public function addUser($username, $email, $password, $firstname, $lastname, $birthdate) {
        ...
        if($result->num_rows > 0) {
            ...
            //convert the password into on encrypted text
    ...
    public function updateUser($user_id, $username, $email, $firstname, $lastname, $birthdate) {
        ...
        if($result->num_rows > 0) {
            ...
            $sql = "UPDATE users SET username='$username', email='$email', firstname='$firstname', lastname='$lastname', birthdate='$birthdate' WHERE user_id=$user_id";
            $result = $this->conn->query($sql);
            if($result == FALSE) {
                echo "ERROR: " . $this->conn->error;
            } else {
